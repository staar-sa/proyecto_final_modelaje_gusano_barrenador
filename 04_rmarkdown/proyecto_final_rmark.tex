...
\includegraphics{images/Captura de pantalla 2025-11-23 152313.png}
...
\def\labelenumi{\arabic{enumi}.}
\item
  Huevos Totales por Hembra (en su vida):\\
  \[
  ...
  \]
\item
  Tasa de Oviposición por Hembra (diaria, durante su vida
  reproductiva):\\
  \[
  \text{Tasa Hembra} = \frac{1200 \text{ huevos}}{14 \text{ días}} \approx 85.7 \text{ huevos/día/hembra}
  \]
\item
  Tasa de Oviposición por Adulto (\(\alpha_{\text{base}}\)):\\
  Dado que el compartimento \(A\) incluye machos y hembras (50\% de cada
  uno),\\
  la tasa promedio por individuo en \(A\) es la mitad de la tasa de la
  hembra. \[
...
\textbf{Suposiciones para las Tasas de Mortalidad}
...
\bottomrule\noalign{}
\endlastfoot
\(\mu_H\) & Huevo & Mínima mortalidad (principalmente por factores
intrínsecos). & 0.005 (0.5\%) & El texto SENASICA 2020 indica que
eclosionan rápidamente (1 día). Los huevos en la masa están protegidos
por la hembra y por estar en el borde de una herida (ambiente
nutritivo). \\
\(\mu_L\) & Larva & Mortalidad baja (protegida dentro del huésped). &
0.01 (1\%) & La larva es un parásito obligado; está protegida dentro del
tejido vivo de la herida. La principal amenaza es el tratamiento (que
está fuera de esta \(\mu\)) o que la herida se seque. \\
\(\mu_P\) & Pupa & Mortalidad más alta (fase ambiental más vulnerable).
& 0.03 (3\%) & La pupa se encuentra en el suelo, donde está expuesta a
la depredación, inundaciones, desecación extrema o temperaturas
desfavorables (aunque el clima base sea óptimo, hay riesgos). \\
\end{longtable}
...
\subsection{\texorpdfstring{Parámetro \(\sigma\) (Susceptible →
Expuesto) - Período
Pre-Erradicación}{Parámetro \textbackslash sigma (Susceptible → Expuesto) - Período Pre-Erradicación}}\label{paruxe1metro-sigma-susceptible-expuesto---peruxedodo-pre-erradicaciuxf3n}

\subsubsection{Paso 1: Cálculo de la proporción de
incidencia}\label{paso-1-cuxe1lculo-de-la-proporciuxf3n-de-incidencia}
...
Población bovina México (\textasciitilde1980-1990) & 24,600,000 cabezas
& INEGI/SAGARPA (1991) \\
Período analizado & 1972 → 1990 (18 años) & \\
\end{longtable}

\textbf{Nota sobre la población:} Se utilizo la población bovina
nacional de México estimada en aproximadamente 24.6 millones de cabezas
durante el período 1980-1990, que representa el punto medio del período
analizado.
...
\subsubsection{Paso 2: Conversión a tasa de
incidencia}\label{paso-2-conversiuxf3n-a-tasa-de-incidencia}

Para obtener la tasa por día, dividimos la proporción entre la duración
del período:

\[\sigma_{\text{pre-erradicación}} = \frac{\text{Proporción de incidencia}}{\text{Duración del período (días)}}\]

\textbf{Datos temporales del período pre-erradicación:}
...
Duración en días & \textbf{6,570 días} \\
\end{longtable}

\textbf{Cálculo de} \(\sigma\) \textbf{pre-erradicación (tasa de
incidencia):}

\[\sigma_{\text{pre-erradicación}} = \frac{1.17 \times 10^{-2}}{6,570 \text{ días}} = 1.78 \times 10^{-6} \text{ día}^{-1}\]

\textbf{Valor que usaremos para el modelo:}
\(\sigma_{\text{pre-erradicación}} \approx 1.78 \times 10^{-6}\)
día\(^{-1}\) (0.00000178 día\(^{-1}\))
...
\item
  De Sanidad Inocuidad Y Calidad Agroalimentaria, S. N. (n.d.).
  \emph{Miasis por Gusano Barrenador}. gob.mx.
  \url{https://www.gob.mx/senasica/documentos/miasis-por-gusano-barrenador?state=published}
\item
  \emph{¿Qué lugar ocupa Chiapas en la producción nacional de carne de
  bovino? - El Heraldo de Chiapas \textbar{} Noticias Locales,
  Policiacas, sobre México, Chiapas y el Mundo}. (n.d.). OEM.
  \url{https://oem.com.mx/elheraldodechiapas/local/que-lugar-ocupa-chiapas-en-la-produccion-nacional-de-carne-de-bovino-23023994}
\end{itemize}
